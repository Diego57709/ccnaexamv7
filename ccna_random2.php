<?php
// Variable para verificar si la contraseña es correcta
$correctPassword = 'sigma';
$showContent = false; // Por defecto, no mostrar el contenido

// Verificar si se ha enviado el formulario con la contraseña
if (isset($_POST['password']) && $_POST['password'] === $correctPassword) {
    $showContent = true; // Contraseña correcta, mostrar contenido
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        ...
        input[type="checkbox"],
        input[type="radio"] {
            margin-right: 10px; /* Ajusta el margen según lo necesites */
        }
...
<body>
<?php
// Mostrar el contenido si la contraseña es correcta
if ($showContent) {
    // Mostrar los contadores y el contenedor de preguntas
    echo "<div id='contadores' class='small'>
        <div>A: 0 F: 0 T: 0</div>
      </div>";

echo "<div class='container mt-3' style='margin-top: 100px !important;'>
        <input type='text' id='filtro' class='form-control' placeholder='Filtrar preguntas...' />
      </div>";

echo "<div id='test-container' class='container mt-5'>
        <!-- Aquí iría el contenido de las preguntas -->
        <p>Contenido de la página - Aquí irían las preguntas del test</p>
      </div>";

} else {
    // Si la contraseña es incorrecta o no se ha ingresado aún, mostrar el formulario
    echo "<h1>Acceso Requerido</h1>";
    echo "<form method='POST' action=''>";
    echo "<input type='password' name='password' placeholder='Introduce la contraseña' required />";
    echo "<input type='submit' value='Acceder' />";
    echo "</form>";

    // Mostrar el mensaje de 'No acceso' si la contraseña es incorrecta
    if (isset($_POST['password']) && $_POST['password'] !== $correctPassword) {
        echo "<div class='no-access'>Acceso Denegado. Contraseña Incorrecta.</div>";
    }
}
?>
    <script>
...
let tiempoIntervalo;
const totalPreguntas = preguntas.filter(pregunta => pregunta.tipo !== "matching").length;
...
function generarPreguntas(filtro = "") {
    const container = document.getElementById("test-container");
    container.innerHTML = ""; // Limpiar preguntas previas

    // Filtrar y mezclar preguntas por el texto ingresado en el input
    let preguntasFiltradas = preguntas.filter(pregunta =>
        pregunta.texto.toLowerCase().includes(filtro.toLowerCase())
    );
    shuffleArray(preguntasFiltradas); // Asegurar que el orden sea aleatorio
    ...
    preguntasFiltradas.forEach(pregunta => {
        ...
        // Si la pregunta tiene una imagen, añadirla
        if (pregunta.imagen) {
        ...
        if (pregunta.tipo === "radio" || pregunta.tipo === "checkbox") {
            ...
            const opcionesDisponibles = [...pregunta.opciones];
            shuffleArray(opcionesDisponibles); // Mezclar las opciones
    ...
    // Iniciar cronómetro si no se ha iniciado previamente
    if (!tiempoIntervalo) iniciarContadorTiempo();
}

// Llamar a la función al cargar la página con todas las preguntas
window.onload = () => generarPreguntas();

// Función para verificar respuestas
function verificar(pregunta) {
    if (pregunta.tipo === "matching") return;
    ...
    // Actualizar contadores y verificar si se debe redirigir
    mostrarContadores();
    verificarRedireccion();
}

// Función para verificar si se debe redirigir al video
function verificarRedireccion() {
    if (aciertos === totalPreguntas) {
        window.location.href = "https://www.youtube.com/watch?v=dQw4w9WgXcQ"; // Cambia esta URL por la del video deseado
    }
}
...
function mostrarContadores() {
    ...
    contadoresDiv.innerHTML = `
        <div>Aciertos: ${aciertos}/${totalPreguntas} | Fallos: ${fallos} | Tiempo: ${tiempoTranscurrido}s</div>
    `;
}
...
// Agregar evento al input de filtro
document.getElementById("filtro").addEventListener("input", function (event) {
    const filtro = event.target.value;
    generarPreguntas(filtro); // Regenerar preguntas filtradas
});
